...
export default function KnownWordsTab() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      console.log('📦 Bulunan döküman sayısı:', snapshot.size);
      ...
      console.log('✅ Toplam kelime:', wordsData.length);
      setKnownWords(wordsData);
    });
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <Text style={styles.title}>Bildiklerim ✅</Text>
        <Text style={styles.subtitle}>Toplam: {knownWords.length} kelime</Text>
        <Text style={styles.hintText}>💡 Düzenlemek için karta uzun basın</Text>
      </View>

      {knownWords.length === 0 ? (
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>Henüz bildiğiniz kelime yok</Text>
          <Text style={styles.emptySubtext}>Çalış sekmesinde "Bildim" butonuna basın</Text>
        </View>
      ) : (
        <FlatList
          data={knownWords}
      ...
      <View style={styles.tabBar}>
        ...
        >
          <Text style={styles.tabIcon}>✅</Text>
          <Text style={[styles.tabText, styles.activeTab]}>Bildiklerim</Text>
        </TouchableOpacity>
